...
| `http://localhost/joker-chat-api/joker-chat/messages/conversation/<odbiorca_id>/<ileWiadomosciWstecz>/` | GET | Zwraca wskazaną liczbę ostatnich wiadomości w rozmowie z użytkownikiem `odbiorca_id` (maks. 100). | Parametry ścieżki: identyfikator odbiorcy i liczba wiadomości do cofnięcia. | `200 OK` z listą wiadomości lub błąd `400/404` przy niepoprawnych danych. |
